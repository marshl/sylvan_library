...
class ManaReplaceTestCase(TestCase):
    ...
    def test_simple_replace(self) -> None:
        ...
        mana_cost = "{R}"
        self.assertEqual(
            '<i class="ms ms-r ms-cost"></i>', replace_mana_symbols(mana_cost)
        )
    ...
    def test_simple_replace_lowercsae(self) -> None:
        ...
        mana_cost = "{u}"
        self.assertEqual(
            '<i class="ms ms-u ms-cost"></i>', replace_mana_symbols(mana_cost)
        )
    ...
    def test_hybrid_replace(self) -> None:
        ...
        mana_cost = "{W/U}"
        self.assertEqual(
            '<i class="ms ms-wu ms-cost"></i>', replace_mana_symbols(mana_cost)
        )
    ...
    def test_large_number_replace(self) -> None:
        ...
        mana_cost = "{11}"
        self.assertEqual(
            '<i class="ms ms-11 ms-cost"></i>', replace_mana_symbols(mana_cost)
        )
