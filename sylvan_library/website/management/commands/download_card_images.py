    ...
    def handle(self, *args, **options):
        ...
        all_printings = (
            CardPrinting.objects.filter(
                printed_languages__image__isnull=True,
                printed_languages__language__code__isnull=False,
                scryfall_id__isnull=False,
            )
            .prefetch_related(
                "set", "printed_languages__language", "printed_languages__image"
            )
            .distinct()
        )
        ...
        for printing in all_printings:
            self.download_images_for_printing(
                printing, download_all_languages=options["download_all_languages"]
            )

        self.image_download_queue.join()

    def download_images_for_printing(
        self, printing: CardPrinting, download_all_languages: bool = False
    ):
        """
        ...
        for printed_language in printing.printed_languages.all():
            ...
            if base_image_uri is None:
                ...
                logger.info(
                    "Queueing images for %s (%s): %s",
                    printing,
                    self.image_download_queue.qsize(),
                    url,
                )
                ...
                if "image_uris" in data:
                    base_image_uri = data["image_uris"]["normal"].replace(
                        "/" + data["lang"] + "/", "/[language]/"
                    )
                elif "card_faces" in data:
                    base_image_uri = next(
                        card_face["image_uris"]["normal"].replace(
                            "/" + data["lang"] + "/", "/[language]/"
                        )
            ...
            localised_image_uri = base_image_uri.replace(
                "/[language]/", f"/{printed_language.language.code}/"
            )
            self.image_download_queue.put(
                (printed_language.id, localised_image_uri, image_path)
            )
        ...
        while True:
            ...
            try:
                ...
                card_image = CardImage(
                    printed_language=printed_language, downloaded=False
                )
                card_image.save()
            else:
                ...
                card_image = CardImage(
                    printed_language=printed_language, downloaded=True
                )
                card_image.save()
                logger.info(
                    "\t%s: Downloaded %s (%s)",
