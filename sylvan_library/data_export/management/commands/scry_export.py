...
class Command(BaseCommand):
    ...
    def add_arguments(self, parser):
        ...
        parser.add_argument('filename', nargs=1, type=str, help='The file to export the data to')
    ...
    def export_oracle(self, output_directory: str, pretty_print: bool):
        ...
        for card in Card.objects \
            ...
            ET.SubElement(card_xml, 'npw').text = str(card.num_power) \
                if card.num_power != math.inf else "0"
            ET.SubElement(card_xml, 'ntf').text = str(card.num_toughness) \
                if card.num_toughness != math.inf else "0"
    ...
    def export_setlist(self, output_directory: str, pretty_print: bool):
        ...
        for block in Block.objects.annotate(date=Min('sets__release_date')).order_by('date'):
