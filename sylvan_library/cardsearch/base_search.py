...
class BaseSearch:
    ...
    def get_page_info(self, current_page, page_span):
        ...
        if current_page - page_span > 1:
            page_info.insert(0, PageInfo(None, False, is_placeholder=True))
            page_info.insert(0, PageInfo(1, True))

        if current_page + page_span <= self.paginator.num_pages:
            page_info.append(PageInfo(None, False, is_placeholder=True))

        page_info.insert(0, PageInfo(1, current_page != 1, is_previous=True))
